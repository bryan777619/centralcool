...
class Reservasi(models.Model):
    ...
    def clean(self):
        ...
        conflicting_reservations = Reservasi.objects.filter(
            tanggal=self.tanggal,
            waktu=self.waktu,
            is_cancelled=False
        ).exclude(pk=self.pk)

        if conflicting_reservations.count() >= self.MAX_RESERVATIONS_PER_SLOT:
            raise ValidationError(f'Slot waktu {self.waktu} pada tanggal {self.tanggal} sudah penuh.')
